...
func (d *DB) CreateBin(b *Bin) error {
	b.ID = genID(); b.CreatedAt = now()
	if b.ResponseCode <= 0 { b.ResponseCode = 200 }
	if b.ResponseType == "" { b.ResponseType = "application/json" }
...
func (d *DB) GetBin(id string) *Bin {
	var b Bin
	if err := d.db.QueryRow(`SELECT id,name,slug,response_code,response_body,response_type,created_at FROM bins WHERE id=?`, id).Scan(&b.ID, &b.Name, &b.Slug, &b.ResponseCode, &b.ResponseBody, &b.ResponseType, &b.CreatedAt); err != nil { return nil }
...
func (d *DB) GetBinBySlug(slug string) *Bin {
	var b Bin
	if err := d.db.QueryRow(`SELECT id,name,slug,response_code,response_body,response_type,created_at FROM bins WHERE slug=?`, slug).Scan(&b.ID, &b.Name, &b.Slug, &b.ResponseCode, &b.ResponseBody, &b.ResponseType, &b.CreatedAt); err != nil { return nil }
...
func (d *DB) GetRequest(id string) *Request {
	var r Request; var hj string
	if err := d.db.QueryRow(`SELECT id,bin_id,method,path,query,headers_json,body,ip,size,created_at FROM requests WHERE id=?`, id).Scan(&r.ID, &r.BinID, &r.Method, &r.Path, &r.Query, &hj, &r.Body, &r.IP, &r.Size, &r.CreatedAt); err != nil { return nil }
	json.Unmarshal([]byte(hj), &r.Headers)
	return &r
}
...
type Stats struct { Bins int `json:"bins"`; Requests int `json:"requests"` }
