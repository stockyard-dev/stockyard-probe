...
func New(db *store.DB) *Server {
	...
	s.mux.HandleFunc("GET /api/requests", s.list)
	s.mux.HandleFunc("POST /api/requests", s.create)
	s.mux.HandleFunc("GET /api/requests/{id}", s.get)
	s.mux.HandleFunc("DELETE /api/requests/{id}", s.del)
...
func (s *Server) health(w http.ResponseWriter, r *http.Request) {
	wj(w, map[string]any{"status": "ok", "service": "stockyard-probe", "time": time.Now().UTC().Format(time.RFC3339)})
}

func (s *Server) stats(w http.ResponseWriter, r *http.Request) {
	wj(w, map[string]any{"requests": s.db.Count()})
}

func (s *Server) list(w http.ResponseWriter, r *http.Request) {
	items := s.db.List()
	if items == nil { items = []store.Request{} }
	wj(w, map[string]any{"requests": items, "count": len(items)})
}
